	...
	function checkStatus() {
		$.ajax({
			...
			success: function (response) {
				...
				switch (status) {
					...
					case 'confirming':
						...
						break;
					case 'partial':
						showStatus('partial', payzcore_params.i18n.partial + ' - Please send the remaining amount to the same address');
						break;
					case 'expired':
					case 'cancelled':
						handleExpired();
			...
			error: function () {
				pollFailCount++;
				if (pollFailCount >= 3) {
					showStatus('partial', payzcore_params.i18n.connection_issue || 'Connection issue. Still trying to check payment status…');
				}
			}
		});
	}
	...
	function bindTxidForm() {
		...
		submitBtn.on('click', function () {
			...
			if (!/^[a-fA-F0-9]{10,128}$/.test(cleanHash)) {
				showTxidMessage('Invalid transaction hash format. Please enter a valid hex hash.', 'error');
				return;
			}
