...
class PayzCore_API {
	 ...
	 *     @type string $chain            Required. Blockchain network (TRC20, BEP20, ERC20, POLYGON, ARBITRUM).
	...
	public function get_payment( $payment_id ) {
		return $this->get( '/api/v1/payments/' . sanitize_text_field( $payment_id ) );
	}
	...
	private function handle_response( $response ) {
		if ( is_wp_error( $response ) ) {
			return new WP_Error(
				...
				sprintf(
					...
					__( 'PayzCore API connection failed: %s', 'payzcore' ),
		...
		if ( json_last_error() !== JSON_ERROR_NONE ) {
			return new WP_Error(
				...
				__( 'Invalid response from PayzCore API.', 'payzcore' )
			);
		}
		...
		$error_message = isset( $decoded['error'] ) ? $decoded['error'] : __( 'Unknown API error.', 'payzcore' );
