...
class PayzCore_Webhook {
	...
	public function handle_webhook( $request ) {
		...
		$chain            = isset( $payload['chain'] ) ? sanitize_text_field( $payload['chain'] ) : '';
		...
		$this->process_event( $order, $payment_event, $paid_amount, $tx_hash, $chain, $token, $payload );
	 ...
	 * @return void
	 */
	private function process_event( $order, $event, $paid_amount, $tx_hash, $chain, $token, $payload ) {
		$explorer_url = $this->get_explorer_url( $tx_hash, $chain );
		...
		switch ( $event ) {
			case 'payment.completed':
				...
				$note = sprintf(
					/* translators: 1: paid amount 2: token name 3: chain name 4: explorer link */
					__( 'Payment confirmed. %1$s %2$s received on %3$s. Transaction: %4$s', 'payzcore' ),
					$paid_amount,
					$token,
					$chain,
				...
				if ( $this->order_is_virtual( $order ) ) {
					$order->set_status( 'completed', __( 'Virtual order auto-completed.', 'payzcore' ) );
				}
				break;
			...
			case 'payment.overpaid':
				...
				$note     = sprintf(
					/* translators: 1: paid amount 2: token name 3: expected amount 4: chain name 5: explorer link */
					__( 'Overpayment detected. %1$s %2$s received (expected %3$s) on %4$s. Transaction: %5$s', 'payzcore' ),
					$paid_amount,
					$token,
					$expected,
					$chain,
				...
				$order->payment_complete( $tx_hash );
				if ( $this->order_is_virtual( $order ) ) {
					$order->set_status( 'completed', __( 'Virtual order auto-completed (overpaid).', 'payzcore' ) );
				}
				break;
			...
			case 'payment.partial':
				$expected = $order->get_meta( '_payzcore_expected_amount', true );
				$note     = sprintf(
					/* translators: 1: paid amount 2: expected amount 3: token name 4: chain name */
					__( 'Partial transfer detected. %1$s of %2$s %3$s received on %4$s. Awaiting remaining amount.', 'payzcore' ),
					$paid_amount,
					$expected,
					$token,
					$chain
			...
			case 'payment.expired':
				$note = __( 'Payment monitoring window expired. No sufficient transfer detected.', 'payzcore' );
				$order->add_order_note( $note );
			...
			case 'payment.cancelled':
				$note = __( 'Payment was cancelled by the merchant.', 'payzcore' );
				$order->add_order_note( $note );
				$order->set_status( 'cancelled', $note );
			...
			default:
				$order->add_order_note(
					sprintf(
						...
						__( 'Received unhandled event: %s', 'payzcore' ),
						$event
					)
				);
				break;
	 ...
	 * @param string $chain   Blockchain network (TRC20, BEP20, ERC20, POLYGON, ARBITRUM).
	 * @return string|false Explorer URL or false if not available.
	 */
	private function get_explorer_url( $tx_hash, $chain ) {
		if ( empty( $tx_hash ) ) {
			return false;
		}
		...
		if ( isset( $explorers[ $chain ] ) ) {
			return $explorers[ $chain ] . $tx_hash;
		}
