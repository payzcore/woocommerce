 ...
 *
 * Supports multiple chains (TRC20, BEP20, ERC20, Polygon, Arbitrum) and
 * tokens (USDT, USDC).
 *
 * @package PayzCore
 */
...
class WC_Gateway_PayzCore extends WC_Payment_Gateway {
	...
	public function __construct() {
		...
		$this->method_title       = __( 'PayzCore (Stablecoin)', 'payzcore' );
	...
	public function is_available() {
		...
		// Require at least one chain from the synced API config.
		if ( empty( $this->get_enabled_chains() ) ) {
			return false;
		}
		return true;
	}
	...
	public function process_admin_options() {
		...
		// Fetch project config from PayzCore API (available chains/tokens).
	 ...
	 *
	 * Called on admin save ("Test Connection"). Stores available chains,
	...
	private function fetch_and_cache_config() {
		...
		$response = wp_remote_get(
			untrailingslashit( $api_url ) . '/api/v1/config',
			array(
				'timeout' => 10,
				'headers' => array(
					'x-api-key'    => $api_key,
		...
		if ( is_wp_error( $response ) ) {
			WC_Admin_Settings::add_error(
				sprintf(
					...
					__( 'PayzCore connection failed: %s. Check the API URL and server connectivity.', 'payzcore' ),
		...
		if ( 401 === $status_code || 403 === $status_code ) {
			WC_Admin_Settings::add_error(
				__( 'PayzCore API Key is invalid or the project is inactive. Check your credentials at app.payzcore.com.', 'payzcore' )
			);
			return false;
		}
		...
		if ( $status_code < 200 || $status_code >= 300 ) {
			WC_Admin_Settings::add_error(
				sprintf(
					...
					__( 'PayzCore API returned HTTP %s. Please try again.', 'payzcore' ),
					$status_code
				)
			);
			return false;
		...
		if ( ! is_array( $body ) ) {
			WC_Admin_Settings::add_error(
				__( 'PayzCore returned an invalid response.', 'payzcore' )
			);
			return false;
		}

		$chains        = isset( $body['chains'] ) ? $body['chains'] : array();
		$default_token = isset( $body['default_token'] ) ? $body['default_token'] : 'USDT';

		// Cache the config.
		$this->update_option( '_cached_chains', $chains );
		$this->update_option( '_cached_default_token', $default_token );
		$this->update_option( '_cached_at', current_time( 'mysql' ) );

		if ( empty( $chains ) ) {
			WC_Admin_Settings::add_error(
				__( 'Connected but no wallet configured in your PayzCore project. Add a wallet at app.payzcore.com to accept payments.', 'payzcore' )
			);
		} else {
			$chain_names = array_map( function( $c ) {
				return $c['name'] . ' (' . implode( ', ', $c['tokens'] ) . ')';
			}, $chains );
			WC_Admin_Settings::add_message(
				sprintf(
					/* translators: 1: chain list 2: sync time */
					__( 'PayzCore connected! Available: %1$s. Last sync: %2$s', 'payzcore' ),
					implode( ', ', $chain_names ),
					current_time( 'mysql' )
				)
			);
	...
	private function get_config_status_html() {
		$cached_chains = $this->get_option( '_cached_chains', array() );
		...
		if ( empty( $api_key ) ) {
			return '<div style="background:#1c1917;border:1px solid rgba(161,161,170,0.2);border-radius:6px;padding:12px 16px;font-size:13px;color:#a1a1aa;">'
				. esc_html__( 'Enter your API Key and click Save to connect with PayzCore.', 'payzcore' )
				. '</div>';
		}
		...
		if ( ! empty( $cached_chains ) && is_array( $cached_chains ) ) {
			...
			return '<div style="background:#022c22;border:1px solid rgba(6,182,212,0.3);border-radius:6px;padding:12px 16px;font-size:13px;color:#d1fae5;">'
				. '<strong style="color:#06b6d4;">' . esc_html__( 'Connected', 'payzcore' ) . '</strong><br>'
				. sprintf(
					/* translators: %s: list of available chains */
					esc_html__( 'Available chains: %s', 'payzcore' ),
					implode( ', ', $chain_parts )
				)
				. '<br>'
				...
				. sprintf(
					...
					esc_html__( 'Last synced: %s', 'payzcore' ),
					$time_display
				)
				. ' &mdash; ' . esc_html__( 'Save settings to re-sync.', 'payzcore' )
				. '</span>'
				. '</div>';
		}
		...
		return '<div style="background:#1c1917;border:1px solid rgba(245,158,11,0.3);border-radius:6px;padding:12px 16px;font-size:13px;color:#fbbf24;">'
			. esc_html__( 'Not synced yet. Click Save to connect and fetch your project configuration from PayzCore.', 'payzcore' )
			. '</div>';
	}
		...
		$this->form_fields = array(
			'setup_guide'    => array(
				...
				'description' => sprintf(
					...
					. '</div>',
					__( 'Before you begin:', 'payzcore' ),
					__( 'Create a PayzCore account at', 'payzcore' ),
					__( 'Create a Project and add a Wallet (HD xPub or static addresses) for the blockchain network you want to use.', 'payzcore' ),
					__( 'Copy your API Key and Webhook Secret from the project settings page.', 'payzcore' ),
					__( 'Set your Webhook URL in the PayzCore project to:', 'payzcore' ),
					esc_html( $webhook_url ),
					__( 'Enable the payment method below and save.', 'payzcore' ),
					__( 'You must have a wallet configured for the selected blockchain network in your PayzCore project. If no wallet is set up, payments will fail at checkout.', 'payzcore' )
				),
			),
			'enabled'        => array(
				'title'   => __( 'Enable/Disable', 'payzcore' ),
				'type'    => 'checkbox',
				'label'   => __( 'Enable PayzCore stablecoin payments', 'payzcore' ),
				'default' => 'no',
			),
			'title'          => array(
				'title'       => __( 'Title', 'payzcore' ),
				'type'        => 'text',
				'description' => __( 'Payment method title shown during checkout.', 'payzcore' ),
				'default'     => __( 'Stablecoin (USDT/USDC)', 'payzcore' ),
				'desc_tip'    => true,
			),
			'description'    => array(
				'title'       => __( 'Description', 'payzcore' ),
				'type'        => 'textarea',
				'description' => __( 'Payment method description shown during checkout.', 'payzcore' ),
				'default'     => __( 'Pay with USDT or USDC stablecoin. Funds go directly to the merchant wallet.', 'payzcore' ),
				'desc_tip'    => true,
			),
			'api_key'        => array(
				'title'       => __( 'API Key', 'payzcore' ),
				'type'        => 'text',
				'description' => sprintf(
					/* translators: %s: dashboard URL */
					__( 'Found in your PayzCore dashboard: %s → Projects → your project → Settings.', 'payzcore' ),
			...
			'config_status'  => array(
				'title'       => __( 'Connection Status', 'payzcore' ),
				'type'        => 'title',
				'description' => $this->get_config_status_html(),
			),
			'webhook_secret' => array(
				'title'       => __( 'Webhook Secret', 'payzcore' ),
				'type'        => 'password',
				'description' => sprintf(
					/* translators: 1: webhook URL 2: dashboard URL */
					__( 'Shown once when you create a project (or regenerate keys). Set your Webhook URL in %2$s to: %1$s', 'payzcore' ),
			...
			'api_url'        => array(
				'title'       => __( 'API URL', 'payzcore' ),
				'type'        => 'text',
				'description' => __( 'PayzCore API endpoint. Change only if using a self-hosted instance.', 'payzcore' ),
				'default'     => PayzCore_API::DEFAULT_BASE_URL,
				'desc_tip'    => true,
			),
			'expiry_time'    => array(
				'title'       => __( 'Payment Expiry (seconds)', 'payzcore' ),
				'type'        => 'number',
				'description' => __( 'Time in seconds before the payment monitoring expires. Minimum 300 (5 min), maximum 86400 (24 hr).', 'payzcore' ),
				'default'     => 3600,
				'desc_tip'    => true,
			...
			'static_address' => array(
				'title'       => __( 'Static Wallet Address (Optional)', 'payzcore' ),
				'type'        => 'text',
				'description' => __( 'If set, all payments will use this fixed address. Customers submit their transaction hash (TxID) after sending. Leave empty to use HD wallet addresses derived automatically by PayzCore.', 'payzcore' ),
				'default'     => '',
				'placeholder' => 'T... / 0x...',
			),
			...
			'text_settings_heading' => array(
				'title'       => __( 'Payment Page Texts', 'payzcore' ),
				'type'        => 'title',
				'description' => __( 'Customize all customer-facing texts. Change these to display your preferred language on the payment page.', 'payzcore' ),
			),
			'text_payment_title' => array(
				'title'   => __( 'Page Title', 'payzcore' ),
				'type'    => 'text',
				'default' => 'Complete Your Payment',
				'desc_tip' => true,
				'description' => __( 'Main heading on the payment page.', 'payzcore' ),
			),
			'text_payment_subtitle' => array(
				'title'   => __( 'Page Subtitle', 'payzcore' ),
				'type'    => 'textarea',
				'default' => 'Send the exact amount below to the provided address',
				'desc_tip' => true,
				'description' => __( 'Subtitle text below the heading.', 'payzcore' ),
			),
			'text_amount_label' => array(
				'title'   => __( 'Amount Label', 'payzcore' ),
				'type'    => 'text',
				'default' => 'Amount to Send',
				'desc_tip' => true,
			),
			'text_amount_warning' => array(
				'title'   => __( 'Amount Warning', 'payzcore' ),
				'type'    => 'text',
				'default' => 'Send the exact amount including cents',
				'desc_tip' => true,
			),
			'text_address_label' => array(
				'title'   => __( 'Address Label', 'payzcore' ),
				'type'    => 'text',
				'default' => 'Wallet Address',
				'desc_tip' => true,
			),
			'text_time_remaining' => array(
				'title'   => __( 'Time Remaining', 'payzcore' ),
				'type'    => 'text',
				'default' => 'Time remaining:',
				'desc_tip' => true,
			),
			'text_step1' => array(
				'title'   => __( 'Step 1 Text', 'payzcore' ),
				'type'    => 'text',
				'default' => 'Copy the address or scan the QR code',
				'desc_tip' => true,
			),
			'text_step2_template' => array(
				'title'   => __( 'Step 2 Text', 'payzcore' ),
				'type'    => 'text',
				'default' => 'Send exactly {amount} {token}',
				'desc_tip' => true,
				'description' => __( 'Use {amount} and {token} as placeholders.', 'payzcore' ),
			),
			'text_step3' => array(
				'title'   => __( 'Step 3 Text', 'payzcore' ),
				'type'    => 'text',
				'default' => 'Wait for blockchain confirmation (automatic)',
				'desc_tip' => true,
			),
			'text_status_waiting' => array(
				'title'   => __( 'Status: Waiting', 'payzcore' ),
				'type'    => 'text',
				'default' => 'Checking blockchain for your transaction',
				'desc_tip' => true,
			),
			'text_status_confirming' => array(
				'title'   => __( 'Status: Confirming', 'payzcore' ),
				'type'    => 'text',
				'default' => 'Transfer detected, confirming...',
				'desc_tip' => true,
			),
			'text_status_confirmed' => array(
				'title'   => __( 'Status: Confirmed', 'payzcore' ),
				'type'    => 'text',
				'default' => 'Payment confirmed!',
				'desc_tip' => true,
			),
			'text_status_expired' => array(
				'title'   => __( 'Status: Expired', 'payzcore' ),
				'type'    => 'text',
				'default' => 'Payment window expired',
				'desc_tip' => true,
			),
			'text_status_partial' => array(
				'title'   => __( 'Status: Partial', 'payzcore' ),
				'type'    => 'text',
				'default' => 'Partial transfer detected',
				'desc_tip' => true,
			),
			'text_copied' => array(
				'title'   => __( 'Copied Button', 'payzcore' ),
				'type'    => 'text',
				'default' => 'Copied!',
				'desc_tip' => true,
			),
			'text_redirecting' => array(
				'title'   => __( 'Redirecting Text', 'payzcore' ),
				'type'    => 'text',
				'default' => 'Redirecting...',
				'desc_tip' => true,
			),
			'text_txid_label' => array(
				'title'   => __( 'Transaction Hash Label', 'payzcore' ),
				'type'    => 'text',
				'default' => 'Transaction Hash (TxID)',
				'desc_tip' => true,
				'description' => __( 'Label for the transaction hash input in static wallet mode.', 'payzcore' ),
			),
			'text_txid_placeholder' => array(
				'title'   => __( 'Transaction Hash Placeholder', 'payzcore' ),
				'type'    => 'text',
				'default' => 'Paste your transaction hash here',
				'desc_tip' => true,
			),
			'text_txid_button' => array(
				'title'   => __( 'Confirm Button Text', 'payzcore' ),
				'type'    => 'text',
				'default' => 'Confirm Payment',
				'desc_tip' => true,
			),
			'text_txid_success' => array(
				'title'   => __( 'Confirmation Success Text', 'payzcore' ),
				'type'    => 'text',
	...
	public function validate_api_key_field( $key, $value ) {
		...
		if ( ! empty( $value ) && strpos( $value, 'pk_' ) !== 0 ) {
			WC_Admin_Settings::add_error(
				__( 'PayzCore API Key should start with "pk_". Please check your credentials.', 'payzcore' )
			);
		}
		return $value;
	}
	 ...
	 *
	 * Shows blockchain network and token selectors when multiple chains are
	 * enabled. When only one chain is configured, hidden fields are used.
	 *
	 * @return void
	 */
	public function payment_fields() {
		...
		$chains = $this->get_enabled_chains();

		if ( empty( $chains ) ) {
			echo '<p style="color:#ef4444;">' . esc_html__( 'Payment configuration is not available. Please contact the store.', 'payzcore' ) . '</p>';
			return;
		}

		// Build per-chain token map from cached config for JS.
		$chain_tokens_map = array();
		foreach ( $chains as $chain ) {
			$chain_tokens_map[ $chain ] = $this->get_chain_tokens( $chain );
		}

		// If only 1 chain with 1 token, hidden fields (no UI needed).
		if ( count( $chains ) === 1 ) {
			$chain  = reset( $chains );
			$tokens = $chain_tokens_map[ $chain ];
			echo '<input type="hidden" name="payzcore_chain" value="' . esc_attr( $chain ) . '" />';
			if ( count( $tokens ) === 1 ) {
				echo '<input type="hidden" name="payzcore_token" value="' . esc_attr( $tokens[0] ) . '" />';
				return;
			}
			// Single chain, multiple tokens: show token selector only.
			$default_token = $this->get_default_token();
			echo '<div class="payzcore-chain-select" style="margin-bottom:12px;">';
			echo '<p class="form-row form-row-wide">';
			echo '<label for="payzcore_token">' . esc_html__( 'Stablecoin', 'payzcore' ) . '</label>';
		...
		// Multiple chains: show chain + token selectors.
		$chain_labels = array(
		...
		echo '<div class="payzcore-chain-select" style="margin-bottom:12px;">';
		echo '<p class="form-row form-row-wide">';
		echo '<label for="payzcore_chain">' . esc_html__( 'Blockchain Network', 'payzcore' ) . ' <abbr class="required" title="required">*</abbr></label>';
		echo '<select name="payzcore_chain" id="payzcore_chain" class="select" style="width:100%;">';
		foreach ( $chains as $chain ) {
			$label = isset( $chain_labels[ $chain ] ) ? $chain_labels[ $chain ] : $chain;
			echo '<option value="' . esc_attr( $chain ) . '">' . esc_html( $label ) . '</option>';
		}
		echo '</select>';
		echo '</p>';

		// Token selector (dynamically updated by JS based on chain selection).
		$default_token = $this->get_default_token();
		...
		(function(){
			...
			chainEl.addEventListener("change", update);
			update();
		})();
		</script>
		<?php
	 ...
	 *
	 * Ensures the selected chain and token are valid according to the
	 * cached API config.
	 *
	 * @return bool
	 */
	public function validate_fields() {
		$chain        = isset( $_POST['payzcore_chain'] ) ? sanitize_text_field( wp_unslash( $_POST['payzcore_chain'] ) ) : '';
		$valid_chains = $this->get_enabled_chains();

		if ( empty( $chain ) || ! in_array( $chain, $valid_chains, true ) ) {
			wc_add_notice( __( 'Please select a valid blockchain network.', 'payzcore' ), 'error' );
			return false;
		}

		$token        = isset( $_POST['payzcore_token'] ) ? sanitize_text_field( wp_unslash( $_POST['payzcore_token'] ) ) : 'USDT';
		$valid_tokens = $this->get_chain_tokens( $chain );
		if ( ! in_array( $token, $valid_tokens, true ) ) {
			wc_add_notice(
				sprintf(
					/* translators: 1: token name 2: chain name */
					__( '%1$s is not available on %2$s. Please select a different token or network.', 'payzcore' ),
					$token,
					$chain
				),
				'error'
	 ...
	 *
	 * Reads chain identifiers from the cached /api/v1/config response.
	 * Returns an empty array when no config has been synced yet.
	 *
	 * @return array List of enabled chain identifiers (e.g. ['TRC20', 'BEP20']).
	 */
	private function get_enabled_chains() {
		$cached = $this->get_option( '_cached_chains', array() );
		if ( ! empty( $cached ) && is_array( $cached ) ) {
			return array_map( function( $c ) {
				return is_array( $c ) ? $c['chain'] : $c;
			}, $cached );
		}
		return array();
	}
	...
	/**
	 * Get the full cached chain config with per-chain token info.
	 *
	 * Returns the raw array from the /api/v1/config response, where each
	 * element has 'chain', 'name', and 'tokens' keys.
	 *
	 * @return array List of chain config objects.
	 */
	private function get_cached_chain_config() {
		$cached = $this->get_option( '_cached_chains', array() );
		if ( ! empty( $cached ) && is_array( $cached ) ) {
			return $cached;
		}
	...
	/**
	 * Get the tokens available for a specific chain from cached config.
	 *
	 * @param string $chain Chain identifier (e.g. 'TRC20', 'BEP20').
	 * @return array List of token identifiers (e.g. ['USDT', 'USDC']).
	 */
	private function get_chain_tokens( $chain ) {
		$config = $this->get_cached_chain_config();
		foreach ( $config as $c ) {
			if ( is_array( $c ) && isset( $c['chain'] ) && $c['chain'] === $chain ) {
	...
	public function process_payment( $order_id ) {
		...
		if ( ! $order ) {
			wc_add_notice( __( 'Order not found. Please try again.', 'payzcore' ), 'error' );
		...
		if ( floatval( $amount ) <= 0 ) {
			wc_add_notice( __( 'Invalid order amount.', 'payzcore' ), 'error' );
			return array( 'result' => 'failure' );
		}

		// Read chain/token from checkout selection (with fallbacks).
		$enabled = $this->get_enabled_chains();
		$chain = isset( $_POST['payzcore_chain'] ) ? sanitize_text_field( wp_unslash( $_POST['payzcore_chain'] ) ) : ( ! empty( $enabled ) ? $enabled[0] : 'TRC20' );
		if ( ! in_array( $chain, $enabled, true ) ) {
			$chain = ! empty( $enabled ) ? $enabled[0] : 'TRC20';
		}
		$token = isset( $_POST['payzcore_token'] ) ? sanitize_text_field( wp_unslash( $_POST['payzcore_token'] ) ) : $this->get_default_token();

		// Ensure token is valid for the selected chain (API-driven).
		$valid_tokens = $this->get_chain_tokens( $chain );
		if ( ! in_array( $token, $valid_tokens, true ) ) {
		...
		if ( is_wp_error( $result ) ) {
			...
			$order->add_order_note(
				sprintf(
					...
					__( 'PayzCore API error [%1$s]: %2$s', 'payzcore' ),
					$error_code,
					$error_message
				)
			);
			...
			if ( $is_wallet_error ) {
				$customer_message = __( 'This payment method is temporarily unavailable. The store has not yet configured the required wallet for the selected blockchain network. Please choose a different payment method or contact the store.', 'payzcore' );
			} elseif ( 'payzcore_auth_error' === $error_code ) {
				$customer_message = __( 'This payment method is temporarily unavailable due to a configuration issue. Please choose a different payment method or contact the store.', 'payzcore' );
			} elseif ( 'payzcore_rate_limit' === $error_code ) {
				$customer_message = __( 'Too many requests. Please wait a moment and try again.', 'payzcore' );
			} else {
				$customer_message = __( 'Unable to initiate payment. Please try again or choose a different payment method.', 'payzcore' );
			}
		...
		if ( empty( $result['success'] ) || empty( $result['payment'] ) ) {
			wc_add_notice( __( 'Unexpected response from the payment monitoring service. Please try again.', 'payzcore' ), 'error' );
			return array( 'result' => 'failure' );
		}
		...
		$order->update_meta_data( '_payzcore_chain', sanitize_text_field( $payment['chain'] ) );
		...
		$order->set_status(
			'on-hold',
			sprintf(
				/* translators: 1: amount 2: token name 3: chain name 4: address */
				__( 'Awaiting %1$s %2$s transfer on %3$s to %4$s', 'payzcore' ),
				$payment['amount'],
				$token_name,
				$payment['chain'],
				$payment['address']
	...
	public function thankyou_page( $order_id ) {
		...
		$chain            = $order->get_meta( '_payzcore_chain', true );
	...
	public function email_instructions( $order, $sent_to_admin, $plain_text = false ) {
		...
		if ( $plain_text ) {
			echo "\n";
			echo "==========\n";
			echo esc_html__( 'PAYMENT INSTRUCTIONS', 'payzcore' ) . "\n";
			echo "==========\n\n";
			printf(
				/* translators: 1: amount 2: token name */
				esc_html__( 'Amount: %1$s %2$s', 'payzcore' ) . "\n",
				esc_html( $expected_amount ),
				esc_html( $token )
			);
			printf(
				/* translators: %s: blockchain network */
				esc_html__( 'Network: %s', 'payzcore' ) . "\n",
				esc_html( $chain )
			);
			printf(
				/* translators: %s: wallet address */
				esc_html__( 'Address: %s', 'payzcore' ) . "\n",
				esc_html( $address )
			);
			if ( ! empty( $expires_at ) ) {
				printf(
					/* translators: %s: expiry date/time */
					esc_html__( 'Expires: %s', 'payzcore' ) . "\n",
					esc_html( $expires_at )
				);
			}
			echo "\n" . esc_html__( 'Send the exact amount shown above to the address provided. Your order will be confirmed automatically once the transfer is detected on the blockchain.', 'payzcore' ) . "\n\n";
		} else {
			?>
			<div style="margin: 16px 0; padding: 16px; background: #1a1a2e; border: 1px solid rgba(6,182,212,0.3); border-radius: 8px; color: #e4e4e7;">
				<h3 style="margin: 0 0 12px; color: #06b6d4; font-size: 16px;">
					<?php esc_html_e( 'Payment Instructions', 'payzcore' ); ?>
				</h3>
				<table style="width: 100%; border-collapse: collapse;">
					<tr>
						<td style="padding: 4px 8px 4px 0; color: #a1a1aa; font-size: 13px;"><?php esc_html_e( 'Amount:', 'payzcore' ); ?></td>
					...
					<tr>
						<td style="padding: 4px 8px 4px 0; color: #a1a1aa; font-size: 13px;"><?php esc_html_e( 'Network:', 'payzcore' ); ?></td>
						<td style="padding: 4px 0; font-size: 14px; color: #ffffff;">
							<?php echo esc_html( $chain ); ?>
						</td>
					</tr>
					<tr>
						<td style="padding: 4px 8px 4px 0; color: #a1a1aa; font-size: 13px;"><?php esc_html_e( 'Address:', 'payzcore' ); ?></td>
				...
				<p style="margin: 12px 0 0; font-size: 12px; color: #a1a1aa;">
					<?php esc_html_e( 'Send the exact amount shown above to the address provided. Your order will be confirmed automatically once the transfer is detected on the blockchain.', 'payzcore' ); ?>
				</p>
			</div>
			<?php
		}
		...
		wp_localize_script(
			...
			array(
				...
				'i18n'           => array(
					...
					'copy_failed'      => __( 'Copy failed', 'payzcore' ),
					...
					'status_error'     => __( 'Unable to check status', 'payzcore' ),
					'txid_success'     => wp_kses( $this->get_option( 'text_txid_success', 'Transaction submitted. Awaiting blockchain confirmation.' ), array() ),
					'txid_error'       => __( 'Failed to submit transaction hash. Please try again.', 'payzcore' ),
					'txid_empty'       => __( 'Please enter a transaction hash.', 'payzcore' ),
					'txid_submitting'  => __( 'Submitting...', 'payzcore' ),
				),
			)
		);
	}
	...
	public function ajax_check_status() {
		...
		if ( ! $order_id ) {
			wp_send_json_error( array( 'message' => __( 'Invalid order.', 'payzcore' ) ) );
		}

		$order = wc_get_order( $order_id );

		if ( ! $order ) {
			wp_send_json_error( array( 'message' => __( 'Order not found.', 'payzcore' ) ) );
		}

		$order_key = isset( $_POST['order_key'] ) ? sanitize_text_field( wp_unslash( $_POST['order_key'] ) ) : '';
		if ( $order->get_order_key() !== $order_key ) {
			wp_send_json_error( array( 'message' => __( 'Invalid order key.', 'payzcore' ) ) );
		}
		...
		if ( empty( $payment_id ) ) {
			wp_send_json_error( array( 'message' => __( 'Payment ID not found.', 'payzcore' ) ) );
		}
	...
	public function ajax_confirm_txid() {
		...
		if ( ! $order_id || empty( $tx_hash ) ) {
			wp_send_json_error( array( 'message' => __( 'Missing required fields.', 'payzcore' ) ) );
		}
		...
		if ( ! preg_match( '/^[a-fA-F0-9]{10,128}$/', $clean_hash ) ) {
			wp_send_json_error( array( 'message' => __( 'Invalid transaction hash format.', 'payzcore' ) ) );
		}
		...
		if ( empty( $confirm_endpoint ) ) {
			wp_send_json_error( array( 'message' => __( 'Confirmation endpoint not available for this order.', 'payzcore' ) ) );
		}
		...
		$order->add_order_note(
			sprintf(
				...
				__( 'Customer submitted transaction hash: %s', 'payzcore' ),
				$tx_hash
			)
		);
		$order->save();

		wp_send_json_success( array( 'message' => __( 'Transaction hash submitted successfully.', 'payzcore' ) ) );
	}
}
